define(['Util/util'], function (Util) {
	 ...
	 */
	function toggleBanner(id,len,pid, vm) {
		// console.log(vm);
		var bn = {
			vm			 : vm ? vm : '',
			len 		 : len || 6,
		...
		function touchEnd (e) {
			e.stopPropagation();
			// 触摸结束后现将vm当前的index值与轮播图的index同步，再去进行增减操作
			bn.vm ? bn.index = bn.vm.index : '';
			...
			// bn的index操作完毕后，再次同步到vm的index上
			bn.vm ? bn.vm.index = bn.index : '';
			// 优先使用bn.vm.index
			bn.toggleByIndex(bn.vm ? bn.vm.index : bn.index);
			// 如果有小圆点元素，才会执行切换操作
		...
		function toggleByIndex(index) {
			// console.log('vue.Index: '+ bn.vm.index+'-----bn.index: '+ bn.index+'-----index: '+ index)
			// 计算最终偏移量
